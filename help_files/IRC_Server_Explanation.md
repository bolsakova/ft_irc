...
Порт валидируется при создании `Server` (в `parse_port_strict` внутри `initSocket`): допустим только диапазон 1024–65535. Почему именно этот диапазон? Порты 0–1023 зарезервированы системой и обычно требуют root-прав.

Проверяется, что пароль не пустой.

После всех проверок создается объект **Server**, передавая ему порт и пароль.

Далее устанавливаются обработчики сигналов SIGINT (Ctrl+C) и SIGTERM (команда kill). Когда приходит сигнал, `signalHandler` **не** вызывает сложную логику остановки, а только поднимает флаг `g_stop_requested = 1` (async-signal-safe подход).

Фактическая остановка выполняется уже в обычном потоке внутри `Server::run()`: если `poll()` прерван с `EINTR` и флаг выставлен, вызывается `stop()`.
...
```cpp
while (m_running) {
    if (m_poll_fds.empty()) -> ошибка/выход

    // Один вызов poll() для ВСЕХ сокетов
    poll(m_poll_fds.data(), m_poll_fds.size(), -1);

    if (poll < 0 && errno == EINTR) {
...
- recv() возвращает -1 с другой ошибкой — помечаем клиента на отложенное отключение
...
Если клиент закрыл входящую сторону (`recv == 0`), сервер ставит `peer_closed`, выключает `POLLIN`, и закрывает сокет только после отправки оставшегося `outbuf`.
...
4. send() может отправить:
   ...
   - Часть данных — нормально, удаляем отправленную часть из буфера
    ...
    - Ошибка — помечаем клиента на отложенное отключение

5. Если буфер опустел — отключаем POLLOUT (не нужно больше мониторить запись)
6. Если клиент закрыл соединение (peer_closed) и буфер пустой — помечаем на отключение; фактическое закрытие делает `cleanupDisconnectedClients()`
...
Когда приходит SIGINT/SIGTERM, `signalHandler` поднимает `g_stop_requested`. Затем `poll()` прерывается (`EINTR`), и в `run()` вызывается `server.stop()`:
